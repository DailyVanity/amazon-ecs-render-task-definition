...
async function run() {
  try {
    ...
    const secretVariables = core.getInput('secret-variables', { required: false });
    ...
    if (secretVariables) {
      ...
      secretVariables.split('\n').forEach(function (line) {
        ...
        // If there's nowhere to split
        if (separatorIdx === -1) {
            throw new Error(`Cannot parse the secret variable '${trimmedLine}'. Environment variable pairs must be of the form NAME=value.`);
        }
        // Build object
        const variable = {
          name: trimmedLine.substring(0, separatorIdx),
        ...
        // Search container definition environment for one matching name
        const variableDef = containerDef.secrets.find((e) => e.name == variable.name);
        if (variableDef) {
          // If found, update
          variableDef.value = variable.value;
        } else {
          // Else, create
          containerDef.secrets.push(variable);
        }
      })
    }
